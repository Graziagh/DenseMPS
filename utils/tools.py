...
def makeLogFile(filename="lossHistory.txt"):
    ...
    with open(filename,"w") as text_file:
        print('Epoch\tlossTr\taccTr\taucTr\tlossVl\taccVl\taucVl\ttime(s)',file=text_file)
    print("Log file created...")
    return

def writeLog(logFile, epoch, lossTr, accTr, aucTr, lossVl, accVl, aucVl, eTime):
    print('Epoch:{:04d}\t'.format(epoch + 1),
          'lossTr:{:.4f}\t'.format(lossTr),
          'accTr:{:.4f}\t'.format(accTr),
          'aucTr:{:.4f}\t'.format(aucTr),
          'lossVl:{:.4f}\t'.format(lossVl),
          'accVl:{:.4f}\t'.format(accVl),
          'aucVl:{:.4f}\t'.format(aucVl),
