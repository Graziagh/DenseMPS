...
def init_tensor(shape, bond_str, init_method):
    ...
    if not isinstance(init_method, str):  #init_method: ('random_zero', init_std=1e-9, output_dim=5)
    ...
    assert len(shape) == len(bond_str)  #'bond_str': 'slri' 'shape': [label_site=2, bond_dim=5, bond_dim=5, feature_dim=32]
    ...
    if init_method in ['random_eye', 'min_random_eye']:
        ...
        eye_tensor = torch.eye(bond_dims[0], bond_dims[1]).view(eye_shape) #view 类似于 reshape
